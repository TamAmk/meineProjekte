...
class PCViewer:
    def __init__(self, point_cloud=None, gt_boxes=None, gt_boxes_corners=None,  pred_boxes=None, point_features=4, normalize_intensities=False, calib=None):
        if isinstance(point_cloud, str):
            self.point_cloud = utils.load_point_cloud_from_file(point_cloud, point_features=point_features, normalize_intensities=normalize_intensities)
        else:
        ...
        self.gt_boxes_corners = gt_boxes_corners
    ...
    def draw(self, normalize_intensities=False):
        ...
        intensities = 1 - (self.point_cloud[:, 3:].reshape(-1) / (255 if normalize_intensities else 1))
        cm = plt.get_cmap('jet')
        ...
        if self.gt_boxes_corners is not None:
            corner_boxes = self.gt_boxes_corners
            colors = [[0, 1, 0] for _ in range(len(lines))]
